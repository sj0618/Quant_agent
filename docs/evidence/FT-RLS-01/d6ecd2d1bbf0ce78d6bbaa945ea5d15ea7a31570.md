# FT-RLS-01 evidence bundle — `d6ecd2d`
...
```sh
cd ai && … -m pytest -q tests/test_api.py::test_release_readiness_requires_durable_job_store_before_other_dependencies tests/test_db_data_source.py::test_pipeline_data_source_without_dsn_uses_explicit_nonproduction_fixture tests/test_graph_e2e.py::test_rsi_strategy_runs_ready_e2e_without_external_keys tests/test_live_provider_fail_closed.py
```

`exit=0` — `25 passed, 1 warning in 1.85s`.
...
이전 번들(`29eaef2…`)의 축 대응표가 이 행에 이미 존재한다. 같은 대상 선택이
`d6ecd2d`에서도 통과했으므로 그 표를 이 SHA로 이월한다. 원문은
`docs/evidence/FT-RLS-01/29eaef2295d2e2c04c63bbfdd705cbf2d90ce61a.md` 참조.
