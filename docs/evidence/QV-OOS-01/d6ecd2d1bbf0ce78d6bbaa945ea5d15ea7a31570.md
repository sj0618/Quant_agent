# QV-OOS-01 evidence bundle — `d6ecd2d`
...
> 기간·seed·candidate count·OOS result가 artifact에 있음
...
```sh
cd ai && … -m pytest -q tests/test_backtest_optimization.py tests/test_backtest_loop3.py -k "walk_forward or oos or fold"
```

`exit=0` — `6 passed, 41 deselected in 1.90s`.
