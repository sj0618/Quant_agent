# MT-DATA-03 evidence bundle — `d6ecd2d`
...
> minimum data rule·reason·no recommendation PASS
...
```sh
cd ai && … -m pytest -q tests/test_quant_performance.py -k "insufficient or minimum"
```

`exit=0` — `1 passed, 14 deselected in 0.20s`.
