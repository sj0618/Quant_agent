# QV-WRM-01 evidence bundle — `d6ecd2d`
...
> warm-up 미달 input에서 signal·performance unavailable
...
```sh
cd ai && … -m pytest -q tests/test_backtest_optimization.py tests/test_condition_vocabulary.py -k "warmup or warm_up or window"
```

`exit=0` — `5 passed, 55 deselected in 0.21s`.
