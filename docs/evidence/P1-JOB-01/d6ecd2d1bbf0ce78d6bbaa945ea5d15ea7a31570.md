# P1-JOB-01 evidence bundle — `d6ecd2d`
...
```
cd ai && pytest -q tests/test_jobs.py -k 'restart or retry or idempotency'
```
...
```sh
cd ai && … -m pytest -q tests/test_jobs.py -k "restart or retry or idempotency"
```

`exit=5` — ``.

## 이 행의 고정 명령은 PASS를 만들 수 없다

고정 명령을 그대로 실행한 결과는 `exit=5`(pytest: 수집된 테스트 0개)이고 출력은
`13 deselected`다. `tests/test_jobs.py`에 있는 13개 테스트 중 `restart`, `retry`,
`idempotency` 중 어느 것도 이름에 포함한 것이 없다. 실제 restart 테스트는
`tests/test_interrupted_job_reaper.py`에 있다.

따라서 이 행은 **자기 계약 명령으로 수용을 증명할 수 없다.** 번들을 PASS로 적지
않는다. 선행 조치는 둘 중 하나다 — (a) 고정 명령의 대상 파일을 바로잡거나,
(b) `test_jobs.py`에 해당 이름의 테스트를 만든다. 어느 쪽이든 WBS 행 수정이 필요하다.
