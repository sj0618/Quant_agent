# FT-DB-02 evidence bundle — `d6ecd2d`
...
> 정확한 status·reason·retry policy와 UI contract PASS
...
```sh
cd ai && … -m pytest -q tests/test_api.py::test_database_connection_failure_has_a_safe_retryable_envelope tests/test_screening_pipeline_failure_classifier.py::test_connection_timeout_maps_to_safe_failure_bucket
```

`exit=0` — `2 passed, 1 warning in 0.34s`.
...
`docs/evidence/FT-DB-02/29eaef2295d2e2c04c63bbfdd705cbf2d90ce61a.md` 참조.
