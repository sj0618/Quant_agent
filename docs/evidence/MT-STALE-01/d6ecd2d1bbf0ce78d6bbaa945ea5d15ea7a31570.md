# MT-STALE-01 evidence bundle — `d6ecd2d`
...
> stale status·reason·as-of가 전달되고 숫자 0개
...
```sh
cd ai && … -m pytest -q tests/test_quant_performance.py -k "stale"
```

`exit=0` — `2 passed, 13 deselected in 0.21s`.
