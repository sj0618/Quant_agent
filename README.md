...
> 대상 독자: 백엔드/프론트엔드 팀원, 백테스트 엔진 사용자, 프로젝트 멘토
> 핵심 목표: “어떤 데이터를 어디서 조회하면 되는지”를 빠르게 찾고, 로컬 Docker DB에서 바로 재현하는 것
...
Airflow 환경에서는 `airflow/dags/quant_agent_data_engineering.py`가 일일 파이프라인을 정의합니다.
...
- Airflow DAG: `airflow/dags/quant_agent_data_engineering.py`
