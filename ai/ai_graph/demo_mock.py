...
정확히 "거래량 기반 퀀트 전략"이 입력되면 파이프라인을 **끝까지 그대로 실행한 뒤**, 사용자에게 보여줄 최종 ``APIEnvelope``만 고도로 그럴듯한 전략 + 높은
...
트리거 판정은 공백을 지운 뒤의 정확일치다. 트리거 문구 그대로 입력했을 때만 발동하므로
일반 사용자 경로에는 영향이 없다.
...
def demo_mock_active(query: str) -> bool:
    ...
    띄어쓰기만 무시하고 트리거 문구와 **정확히 같을 때만** 발동한다. 앞뒤로 말이 붙으면
    트리거가 아니다.

    한때 부분일치였는데, 그러면 "…전략 말고 RSI로 해줘"처럼 그 전략을 물리는 요청까지
    목업이 떴다. 그것을 거르려고 배제 표현 판정을 넣었더니 이번에는 조건을 다듬는 정상
    요청("우선주는 제외해줘")까지 함께 꺼졌다. 두 방향 모두 정상 입력을 깨뜨렸으므로
    가장자리를 판정하려는 시도를 접고, 시연 문구 하나만 정확히 받는다.
    """
    ...
    if not needle:
        return False
    return any(_collapse(trigger) == needle for trigger in _triggers())
